...
from vmaig_auth.models import VmaigUser,ChargeLog,OprLog
...
#add by levy @20150624
    def account(self,request):
        request.user.usertel = self.request.POST.get("usertel","")
        request.user.useraddr = self.request.POST.get("useraddr","")
        request.user.save()
        oprlog = OprLog.objects.create(
                name = 'VmaigUser',
                customer = request.user,
    ...
    def charge(self,request):
        request.user.useracnt = self.request.POST.get("useracnt","")
        request.user.save()
        oprlog = OprLog.objects.create(
            name = 'VmaigUser',
            customer = request.user,
        ...
        chargelog = ChargeLog.objects.create(
            customer = request.user,
            money = self.request.POST.get("userchg",""),
        )
        oprlog = OprLog.objects.create(
            name = 'VmaigUser',
            customer = request.user,
