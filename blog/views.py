...
from blog.models import Article,Category,Carousel,Column,Nav,News,Country,Theme,Price,Bought
from vmaig_comments.models import Comment
from vmaig_auth.models import VmaigUser,BuyLog,OprLog,ChargeLog
...
class ArticleView(BaseMixin,DetailView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if ip not in visited_ips:
            try:
                ...
                logger.error(u'[ArticleView]访问不存在的影片:[%s]' % en_title)
                raise Http404
            else:
    ...
    def get_context_data(self,**kwargs):
        ...
        if self.request.user.is_superuser:
            ...
            kwargs['Admin_flag'] = 1
        category = Article.objects.get(en_title=en_title).category
        # print category
        kwargs['guess_list'] = Article.objects.filter(category = category,is_top = True)[0:4]

        #影片价格
        try:
            article_id =  Article.objects.get(en_title=en_title).id
            FilmPrice = Price.objects.get(article=article_id)
            kwargs['filmprice'] = FilmPrice.price
        ...
        try:
            article_id =  Article.objects.get(en_title=en_title).id
            Filmbought = Bought.objects.get(article = article_id, customer_id = user.id)
            FilmboughtFlag = True
        except Bought.DoesNotExist:
            FilmboughtFlag = False

        kwargs['filmboughtflag'] = FilmboughtFlag
    ...
    def post(self, request, *args, **kwargs):
        ...
        filmval = self.request.POST.get("filmval","")
        btn_flag = self.request.POST.get("btn_flag","")

        if btn_flag == "buyfilm":
            # print "buyfilm"
            bought = Bought.objects.create(
                article = Article.objects.get(en_title=filmval),
                customer = self.request.user
            )
            #日志记录
            oprlog = OprLog.objects.create(
            ...
            buylog = BuyLog.objects.create(
                ...
                buy_type = '影片',
                price = self.request.POST.get("filmprice",""),
            )
            oprlog = OprLog.objects.create(
                name = 'BuyLog',
                customer = self.request.user,
...
class AllView_new(BaseMixin,ListView):
    template_name = 'blog/all_new.html'
    context_object_name = 'article_list'

    def get_context_data(self,**kwargs):
        kwargs['category_list'] = Category.objects.all()
        kwargs['country_list'] = Country.objects.all()
        kwargs['theme_list'] = Theme.objects.all()
        kwargs['PAGE_NUM'] = PAGE_NUM
        return super(AllView_new,self).get_context_data(**kwargs)

    def get_queryset(self):
        # article_list = Article.objects.filter(status=0)
        article_list = Article.objects.filter(status=0).order_by("-pub_time")[0:PAGE_NUM]
        return article_list

    def post(self, request, *args, **kwargs):
        val = self.request.POST.get("val","")
        sort = self.request.POST.get("sort","time")
        cnty = self.request.POST.get("cnty","")
        thm = self.request.POST.get("thm","")
        start = self.request.POST.get("start",0)
        end = self.request.POST.get("end",PAGE_NUM)
        ...
        if thm == "all":
            thm = ''
        if val == "all":
            article_list = Article.objects.filter(Q(status=0), Q(country__icontains=cnty), Q(theme__icontains=thm)).order_by(sort)[start:end+1]
        else:
            try:
                article_list = Category.objects.get(name=val).article_set.filter(Q(status=0), Q(country__icontains=cnty), Q(theme__icontains=thm)).order_by(sort)[start:end+1]
            except Category.DoesNotExist:
                logger.error(u'[AllView_new]此分类不存在:Category[%s]'  %val)
                logger.error(u'[AllView_new]此分类不存在:Country[%s]'  %cnty)
                logger.error(u'[AllView_new]此分类不存在:Theme[%s]'  %thm)
                raise PermissionDenied
        isend = len(article_list) != (end-start+1)
...
class UserView(BaseMixin,TemplateView):
    ...
    def get(self,request,*args,**kwargs):
        ...
        if slug == 'changetx':
            ...
            self.template_name = 'blog/user_account.html'
            return super(UserView,self).get(request,*args,**kwargs)
        elif slug == 'charge':
            self.template_name = 'blog/user_charge.html'
            return super(UserView,self).get(request,*args,**kwargs)
        elif slug == 'message':
...
class NewsView(BaseMixin,TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        start_date = datetime.datetime.now()-timedelta(end_day)
        end_date = datetime.datetime.now()-timedelta(start_day)
        # print start_date,end_date
        #获取url中时间断的资讯
        news_list = News.objects.filter(pub_time__range=(start_date, end_date))

        if news_list:
            # print 'new_list_action'
...
class UserHistoryView(BaseMixin,TemplateView):
    template_name = 'blog/user_history.html'

    def get_context_data(self, **kwargs):
        buy_history = BuyLog.objects.filter(customer = self.request.user)[0:20]
        charge_history = ChargeLog.objects.filter(customer = self.request.user)[0:20]
        kwargs['buy_list'] = buy_history
        kwargs['charge_list'] = charge_history
